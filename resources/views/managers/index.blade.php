...
<head>
  ...
  <script src="//cdn.datatables.net/buttons/2.4.2/js/buttons.print.min.js" type="text/javascript"></script> <!-- DataTables Buttons Print -->
   ...
   <div class="container">
    <main class="mt-3">
        <a href="{{ route('managers.create') }}" class="btn btn-primary mb-3">Crear manager</a>
        <table id="mitabla" class="table table-striped">
            <thead>
                <tr>
                    <th>CODIGO. MANAGER</th>
                    <th>NOMBRE. MANAGER</th>
                    <th>TELEFONO. MANAGER</th>
                    <th>CORREO. MANAGER</th>
                    <th>ACCIONES</th>
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $manager['COD_MANAGER'] }}</td>
                    <td>{{ $manager['NOM_MANAGER'] }}</td>
                    <td>{{ $manager['TEL_MANAGER'] }}</td>
                    <td>{{ $manager['COR_MANAGER'] }}</td>

                    <td>
                        <a href="{{ route('managers.edit', $manager['COD_MANAGER']) }}"
                            class="btn btn-info btn-sm">Editar</a>
                        <form action="{{ route('managers.destroy', $manager['COD_MANAGER']) }}" method="POST"
