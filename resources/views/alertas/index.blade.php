...
<head>
  ...
  <script src="//cdn.datatables.net/buttons/2.4.2/js/buttons.print.min.js" type="text/javascript"></script> <!-- DataTables Buttons Print -->
   ...
   <div class="container">
    <main class="mt-3">
        <a href="{{ route('alertas.create') }}" class="btn btn-primary mb-3">Crear alerta</a>
        <table id="mitabla" class="table table-striped">
            <thead>
                <tr>
                    <th>CODIGO.ALERTA</th>
                    <th>CODIGO.CAMPANA</th>
                    <th>TIPO.ALERTA</th>
                    <th>SMS.ALERTA</th>
                    <th>ESTADO.ALERTA</th>
                    <th>PRIORIDAD.ALERTA</th>
                    <th>ACCIONES</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr>
                    <td>{{ $alerta['COD_ALERTA'] }}</td>
                    <td>{{ $alerta['COD_CAMPANA'] }}</td>
                    <td>{{ $alerta['TIP_ALERTA'] }}</td>
                    <td>{{ $alerta['SMS_ALERTA'] }}</td>
                    <td>{{ $alerta['EST_ALERTA'] }}</td>
                    <td>{{ $alerta['PRI_ALERTA'] }}</td>
                    <td>
                        <a href="{{ route('alertas.edit', $alerta['COD_ALERTA']) }}"
                            class="btn btn-info btn-sm">Editar</a>
                        <form action="{{ route('alertas.destroy', $alerta['COD_ALERTA']) }}" method="POST"
