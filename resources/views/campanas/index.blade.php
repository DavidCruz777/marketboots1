...
<head>
  ...
  <script src="//cdn.datatables.net/buttons/2.4.2/js/buttons.print.min.js" type="text/javascript"></script> <!-- DataTables Buttons Print -->
   ...
   <div class="container">
    <main class="mt-3">
        <a href="{{ route('campanas.create') }}" class="btn btn-primary mb-3">Crear Campaña</a>
        <table id="mitabla" class="table table-striped">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $campana['COD_CAMPANA'] }}</td>
                    <td>{{ $campana['COD_RECURSO'] }}</td>
                    <td>{{ $campana['COD_PERSONA'] }}</td>
                    <td>{{ $campana['COD_PRESUPUESTO'] }}</td>
                    <td>{{ $campana['NOM_CAMPANA'] }}</td>
                    <td>{{ $campana['FEC_INICIO'] }}</td>
                    <td>{{ $campana['FEC_FINAL'] }}</td>
                    <td>{{ $campana['DES_CAMPANA'] }}</td>
                    <td>{{ $campana['COD_MANAGER'] }}</td>

                    <td>
                        <a href="{{ route('campanas.edit', $campana['COD_CAMPANA']) }}"
                            class="btn btn-info btn-sm">Editar</a>
                        <form action="{{ route('campanas.destroy', $campana['COD_CAMPANA']) }}" method="POST"
